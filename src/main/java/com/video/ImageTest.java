...
public class ImageTest {
    ...
    public static void main1(String[] args) {
        try {
            // 1. 读取主图片（背景图片）
            File mainImageFile = new File("C:\\Users\\仁联集团\\Desktop\\网页\\1.jpg");
            BufferedImage mainImage = ImageIO.read(mainImageFile);

            // 2. 读取 logo 图片
            File logoImageFile = new File("C:\\Users\\仁联集团\\Desktop\\网页\\logo.png");
            BufferedImage logoImage = ImageIO.read(logoImageFile);

            // 3. 创建 Graphics2D 对象，用于在主图片上绘图
            Graphics2D g2d = mainImage.createGraphics();

            // 4. 设置抗锯齿（提高文字和图片的绘制质量）
            ...
            int sloganTextWidth = fmSlogan.stringWidth(sloganText);
            int sloganTextHeight = fmSlogan.getHeight();
            int sloganTextX = (mainImage.getWidth() - sloganTextWidth) / 2; // 水平居中
            int sloganTextY = mainImage.getHeight() - 50; // 距离底部 50 像素

            // 添加鸡汤文案背景（增强可读性）
            g2d.setColor(new Color(0, 0, 0, 0)); // 半透明黑色背景
            g2d.fillRoundRect(sloganTextX - 10, sloganTextY - sloganTextHeight + 5, sloganTextWidth + 20, sloganTextHeight, 20, 20);

            // 绘制鸡汤文案
            g2d.setColor(Color.WHITE); // 重新设置文字颜色
            g2d.drawString(sloganText, sloganTextX, sloganTextY);
